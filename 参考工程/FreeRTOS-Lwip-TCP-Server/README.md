...
额外值得注意的一点是如果是使用PC比如说MATLAB去和板子传数据，那么板子的接收部分函数直接填想接收的数据量就可以，Lwip在这种情况下可以自动分包，但要是和不完善的TCP协议比如ROS去交互数据，此时Lwip无法自动分包，单次recv函数的最大接收数据量是MSS，一般为1446字节，此时就需要循环接收，在代码中我提供了参考，取消注释即可使用
## 注意事项
